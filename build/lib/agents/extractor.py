...
class ExtractionRouter:
    ...
    def _calculate_confidence(self, doc: ExtractedDocument, strategy: str) -> float:
        ...
        for p in doc.pages:
            ...
            if strategy == "fast_text" and p.tables_count > 0:
                score -= 0.2 # FastText is bad for tables
            
            if strategy == "layout_aware" and p.tables_count > 0:
                score += 0.1 # LayoutAware is good for tables
        ...
        if not doc.full_text or len(doc.full_text.strip()) < 100:
            total_score *= 0.5 # Severe penalty for empty output
    ...
    def _log_ledger(self, strategy: str, confidence: float, doc: ExtractedDocument):
        entry = {
